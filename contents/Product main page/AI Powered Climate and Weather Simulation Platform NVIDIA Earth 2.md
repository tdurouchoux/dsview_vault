---
already_read: true
link: https://www.nvidia.com/en-us/high-performance-computing/earth-2/
read_priority: 0
relevance: 4
source: AI Secret
tags:
...
Key components of the Earth-2 model family include:

1. **Earth-2 Medium Range**: Delivers high-accuracy predictions for over 70 weather variables up to 15 days in advance, outperforming leading open models like GenCast on key industry benchmarks.

2. **Earth-2 Nowcasting**: Uses generative AI to predict satellite and radar imagery, providing high-fidelity, zero- to six-hour hazardous weather forecasts in minutes. It is the first AI model to outperform traditional physics-based systems in simulating storm dynamics.

3. **Earth-2 Global Data Assimilation (Coming Soon)**: Generates precise initial atmospheric conditions in seconds on GPUs rather than hours on supercomputers. When coupled with Earth-2 Medium Range, it delivers the most skillful forecasting predictions produced by an open, entirely AI pipeline.

4. **Earth-2 CorrDiff**: Enables generative AI downscaling 500x faster with a 10,000x improvement in energy efficiency, augmenting current applications and workflows to generate more datasets for better probabilistic distributions of weather events.

5. **Earth-2 FourCastNet 3**: Accelerates AI-based global weather forecasting, allowing enterprises to develop solutions using datasets that are up to 20x larger, capture extreme weather events while maintaining or boosting energy efficiency.
